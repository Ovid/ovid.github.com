...
my $repo_url  = 'https://github.com/Ovid/tramp-freighter.git';
my $clone_dir = 'tramp-freighter';
my $dest_dir  = 'paad/tramp-freighter';

# Clone or pull the tramp-freighter repo
if ( -d $clone_dir ) {
...
local $ENV{VITE_BASE_PATH} = "/$dest_dir/";
system( 'npm', '--prefix', $clone_dir, 'run', 'build' ) == 0
...
say "Done. Tramp Freighter site deployed to $dest_dir/";
