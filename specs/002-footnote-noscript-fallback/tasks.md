...
- [X] T009 [P] [US3] Add test in t/ovid_plugin.t: "noscript section should exist in footer output"
- [X] T010 [P] [US3] Add test in t/ovid_plugin.t: "noscript tag should wrap footnotes section"
- [X] T011 [P] [US3] Add test in t/ovid_plugin.t: "dialog overlay and dialog elements should be outside noscript tag"

### Implementation for User Story 3

- [X] T012 [US3] Modify root/include/footer.tt to wrap footnotes section in `<noscript>` tag (after dialog elements)
- [X] T013 [US3] Add `<aside class="footnotes">` section in root/include/footer.tt within noscript tag
- [X] T014 [US3] Ensure dialog overlay and dialog elements remain outside noscript tag in root/include/footer.tt
- [X] T015 [US3] Verify HTML structure: dialogs first, then noscript footnote section in root/include/footer.tt
...
### BUG-001: Overlay Visible on Page Load with JavaScript Enabled (2025-11-09)

**Issue**: When JavaScript is enabled, the dialog overlay appeared on page load and blocked all page content until the JavaScript code ran and set `aria-hidden="true"`. This created a flash of blocking overlay that made the page temporarily unreadable.

**Root Cause**: The CSS rules in `static/css/dialog.css` show the overlay when `aria-hidden` is not set to `"true"`:
...
However, the initial HTML didn't include `aria-hidden="true"` on:
1. The `<div class="dialog-overlay">` element in `root/include/footer.tt`
2. The `<div class="dialog">` elements generated by `lib/Template/Plugin/Ovid.pm`

**Fix Applied**:
1. Added `aria-hidden="true"` to the dialog overlay in `root/include/footer.tt` (line 109)
2. Added `aria-hidden="true"` to the dialog element template in `lib/Template/Plugin/Ovid.pm` (line 142)

**Files Changed**:
- `root/include/footer.tt`: Added `aria-hidden="true"` to dialog-overlay div
- `lib/Template/Plugin/Ovid.pm`: Added `aria-hidden="true"` to dialog div in add_note() method

**Verification**:
- Tests pass: `prove -l t/ovid_plugin.t` ✓
- Site rebuilds: `perl bin/rebuild` ✓
- Generated HTML includes `aria-hidden="true"` on both overlay and dialog elements ✓

**Impact**: This was a critical bug in User Story 3 implementation. The fix ensures that with JavaScript enabled:
- The overlay is hidden by default via `aria-hidden="true"`
- The Dialog.js code runs and maintains `aria-hidden="true"` until a footnote is clicked
