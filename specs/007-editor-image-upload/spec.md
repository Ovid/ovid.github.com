...
As a writer using the in-browser editor, I can click "Upload Image" beside "Change File", select or paste a PNG/GIF/JPG, add filename plus optional source/alt/caption, and on Save the system resizes the asset below the allowed maximum, stores it under `root/static/images/`, and inserts the templated include snippet at my cursor position with HTML-escaped values.
...
1. **Given** the editor is open on an article, **When** I click "Upload Image" and choose a PNG whose encoded size is under the 300000-byte maximum, **Then** the image saves under the provided filename and a snippet with my metadata appears at the cursor.
2. **Given** I paste a JPG from clipboard and supply optional source/alt/caption, **When** I click Save, **Then** the image is downscaled or recompressed just below the allowed byte limit and the snippet reflects my escaped metadata values.
...
- **FR-005**: The editor UI MUST add an "Upload Image" button to the right of "Change File" and display a modal/form that accepts file selection or clipboard paste for PNG/GIF/JPG content.
...
- **FR-007**: On Save, the system MUST validate type, capture the image bytes, and downscale or recompress to just below `MAX_IMAGE_SIZE_BYTES` (300000) while preserving aspect ratio, skipping upscaling for smaller images.
...
- Resizing "just below" the maximum means the helper repeatedly scales or adjusts compression while keeping aspect ratio until the encoded file size is <= `MAX_IMAGE_SIZE_BYTES` (300000); smaller inputs remain unchanged.
