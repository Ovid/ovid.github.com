        ...
        sub createChatCompletion {
            my $mock_result = bless {
                _res => bless {
                    _success => 1,
                    _json    =>
                      { choices => [ { message => { content => 'A beautiful test image showing a sample scene.' } } ] }
                },
                'MockHTTPResponse'
              },
              'MockOpenAIResult';
    ...
    package MockOpenAIClient2 {
        ...
        sub createChatCompletion {
            return bless {
                _res => bless {
                    _success => 1,
                    _json    => { choices => [ { message => { content => 'A JPEG image description.' } } ] }
                },
                'MockHTTPResponse2'
              },
              'MockOpenAIResult2';
...
# Test _extract_description with successful response
{
    my $mock_res = bless {
        _success => 1,
        _json    => { choices => [ { message => { content => 'Extracted description text' } } ] }
      },
      'MockHTTPResponse';
